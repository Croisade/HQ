...
- **HTTPS certs enabled** (admin console → DNS → HTTPS Certificates): lets `tailscaled` auto-provision genuine Let's Encrypt certs for this device's tailnet DNS name (`server.taild1dee8.ts.net`), via `tailscale cert` / `tailscale serve`. Used to give [navidrome](../navidrome/README.md) a properly-trusted HTTPS endpoint for non-browser Android clients, which can't be told to trust the `*.thegarden` mkcert CA the way browsers can (same class of problem as [KOReader/bindery](../bindery/README.md)). `tailscale serve` runs on a non-standard port (`8443`, not `443`) because [traefik](../traefik/README.md)'s Docker port binding already claims `0.0.0.0:443` host-wide, including the tailscale interface.
